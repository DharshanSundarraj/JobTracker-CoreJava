...
⚠️ Performance Note (Cold Starts): > The backend Java Engine is deployed on a free-tier Render instance, which spins down after periods of inactivity. If the dashboard takes ~50 seconds to load data initially, the server is simply "waking up." Once active, subsequent requests and data mutations are near-instantaneous.
...
Opening and securely managing manual database connections across cloud environments.

Parsing raw JSON payloads from HTTP InputStreams without external libraries.

Managing HTTP Headers, Status Codes, and REST methodologies natively.

Writing multi-stage Dockerfiles to containerize and deploy Java applications.
...
Execute the provided SQL schema in your local MySQL instance to create the applications table.

Set up local environment variables (e.g., DB_PASSWORD) on your machine.

Ensure your DatabaseConnection.java points to your local database URL.

Compile and run JobTrackerAPI.java (or use mvn clean package) to spin up the server locally.

Open index.html in a local browser (ensure the API_URL in the JavaScript file points to http://localhost:8080).
